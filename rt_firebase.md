...
2. Replace all content with the rules from [REALTIME_RULES.json](REALTIME_RULES.json) (preferred) or the rules from [Security Rules](#security-rules) section below
3. Click **Publish**
...
**Note:** The canonical, deployable rules JSON for this repository is available at [REALTIME_RULES.json](REALTIME_RULES.json). Use that file when pasting into the Firebase Console or using the Firebase CLI.
  ...
  "rules": {
      ...
      "$uid": {
        ...
        "folders": {
          "$folderId": {
            ...
            "icon": { ".validate": "newData.isString() && newData.val().length <= 30" },
            "messages": {
              "$messageId": {
        ...
        "devices": {
          "$deviceId": {
            ...
            "type": { ".validate": "newData.val() === 'desktop' || newData.val() === 'mobile' || newData.val() === 'tablet'" },
    ...
    "qr-sessions": {
      "$sessionId": {
        ...
        ".validate": "$sessionId.length > 0",
        "type": { ".validate": "newData.isString()" },
        "userId": { ".validate": "newData.isString() || !newData.exists() || newData.val() === null" },
        "deviceId": { ".validate": "newData.isString()" },
        ...
        "scanRequest": { ".validate": "newData.isObject() || !newData.exists()" },
        "approvedAt": { ".validate": "newData.isNumber() || !newData.exists()" },
        "rejectedAt": { ".validate": "newData.isNumber() || !newData.exists()" }
      }
    },
    "globalMessages": {
      ".read": true,
      ...
      "$msgId": {
        ...
        "uid": { ".validate": "!newData.exists() || newData.isString() || newData.val() === null" },
        ...
        "photoURL": { ".validate": "!newData.exists() || (newData.isString() && newData.val().length > 0 && newData.val().length <= 500)" },
        "clientId": { ".validate": "!newData.exists() || (newData.isString() && newData.val().length > 0)" },
        "category": { ".validate": "!newData.exists() || (newData.isString() && (newData.val() === 'general' || newData.val() === 'resources' || newData.val() === 'help' || newData.val() === 'ideas' || newData.val() === 'links' || newData.val() === 'announcements'))" }
      }
    },
...
### Test 8: Global Chat & QR Session Flow (Guest + Auth)

1. Deploy rules: Open Firebase Console → Realtime Database → Rules and paste the contents of `REALTIME_RULES.json` and click **Publish**.

2. Global Chat (Guest):
   - Open the app in an incognito window (not signed in).
   - Open **Global Share** and post a message.
   - Verify a new child exists under `/globalMessages` with `content`, `clientId`, `timestamp`, `name`, and `category`. There should be no `uid` field for guest posts.
   - Confirm the message appears immediately in the shared messages list for all users.

3. Global Chat (Authenticated):
   - Sign in with a test user.
   - Post a message from the Global Share modal.
   - Verify the created message contains `uid`, `name`, `timestamp`, `photoURL` (if available), `category`, and `clientId` (optional).
   - Confirm that authenticated messages also appear for all users in real-time.

4. QR Session Approval:
   - From any client, create a QR session (the creator may be unauthenticated). Confirm the session object exists at `/qr-sessions/{sessionId}` with `createdAt`, `expiresAt` and `status: 'pending'`.
   - From another client that is signed in, update the session to set `status: 'approved'`, `userId: auth.uid`, and `approvedAt` (or let the app do it). This write should succeed only if `now < expiresAt` and the session was in `pending` state.
   - If the session is expired (`expiresAt < now`), approval attempts should be rejected.

**Expected Result**: Global chat works for both guests and authenticated users; anonymous clients can create but not modify messages; QR sessions can be approved only by authenticated users before expiry.
